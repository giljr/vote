...
var PollForm = React.createClass({
  ...
  handleSubmit: function(e){
    ...
    $.ajax({
      ...
      success: function(data){
        // Open the public page immediately so the organiser can display or
        // share its QR code with the audience.
        window.location.assign('/vote/' + data.poll_id);
      }.bind(this),
      error: function(xhr, status, err){
...
var LivePreview = React.createClass({
  ...
  voteHandler: function(e){
    ...
    //vote immediately, no extra confirmation step
...
var AllPolls = React.createClass({
  ...
  componentDidMount: function(){
    ...
    // atualiza os percentuais automaticamente, para refletir votos de outros usuarios
    this.pollInterval = setInterval(this.loadPollsFromServer, 10000);
  },
